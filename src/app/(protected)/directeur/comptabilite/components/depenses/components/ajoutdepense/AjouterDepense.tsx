...
const AjouterDepense: React.FC<AjouterDepenseProps> = ({ 
  isOpen, 
  onClose, 
  onSave,
  id_ecole,
  ...
  const [formData, setFormData] = useState({
    ...
    statut: 'payé' as 'payé' | 'en attente'
  });
    ...
    <div className="fixed inset-0 backdrop-blur-sm bg-white/30 flex items-start justify-center z-50 text-gray-800 overflow-y-auto pt-10 pb-10">
      <div className="bg-white rounded-lg shadow-lg w-full max-w-md my-auto">
        <div className="flex justify-between items-center p-4 border-b">
          <h2 className="text-lg font-semibold text-gray-800">Ajouter une dépense</h2>
          <button 
            onClick={onClose}
              ...
              <div className="flex items-center">
                <input
                  ...
                  onChange={(e) => {
                    setFormData({
                      ...formData,
                      statut: e.target.checked ? 'payé' : 'en attente'
                    });
                  }}
                  className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                />
                <label htmlFor="statut-paye" className="ml-2 block text-sm text-gray-700">
                  Dépense déjà payée
                </label>
              </div>
              <p className="text-xs text-gray-500 mt-1">
                {formData.statut === 'payé' ? 'Cette dépense est marquée comme payée' : 'Cette dépense est en attente de paiement'}
              </p>
            </div>

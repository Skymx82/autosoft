...
interface ModifierDepenseProps {
  showModal: boolean;
  onClose: () => void;
  depenseId: string;
  id_ecole?: string;
...
export default function ModifierDepense({
  ...
  const [formData, setFormData] = useState({
    ...
    fournisseur: '',
    modePaiement: '',
    statut: 'payé' as 'payé' | 'en attente' | 'annulé'
  });

  // Récupérer les détails de la dépense
  useEffect(() => {
    const fetchDepenseDetails = async () => {
      if (!depenseId || !showModal) return;
      ...
      try {
        ...
        // Appel API pour récupérer les détails de la dépense
        const response = await fetch(`/directeur/comptabilite/components/depenses/components/DetailDepense/api/detail?id_depense=${depenseId}&id_ecole=${ecole_id}&id_bureau=${bureau_id}`);
        ...
        const tauxTVA = data.depense.montant > 0 
          ? Math.round((data.depense.tva / data.depense.montant) * 100) 
          : 20;
        
        // Initialiser le formulaire avec les données de la dépense
        setFormData({
          date: data.depense.date,
          categorie: data.depense.categorie,
          description: data.depense.description,
          montant: data.depense.montant,
          tva: data.depense.tva,
          tauxTVA: tauxTVA,
          fournisseur: data.depense.fournisseur || '',
          modePaiement: data.depense.modePaiement,
          statut: data.depense.statut
        });
        
      } catch (err) {
        console.error('Erreur lors de la récupération des détails de la dépense:', err);
        setError('Impossible de charger les détails de la dépense. Veuillez réessayer.');
      } finally {
        setLoading(false);
      }
    };
    ...
    fetchDepenseDetails();
  }, [depenseId, showModal, id_ecole, id_bureau]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      formDataToSend.append('id_depense', depenseId);
      ...
      const response = await fetch(`/directeur/comptabilite/components/depenses/components/DetailDepense/api/modifier`, {
        method: 'PUT',
        body: formDataToSend
      });
      ...
      setError(err.message || 'Une erreur est survenue lors de la mise à jour de la dépense');
    } finally {
      setSaving(false);
    }
  };
      ...
      >
        ...
        <div className="flex justify-between items-center p-4 border-b border-gray-200 bg-gray-50">
          <div className="flex items-center">
            <h3 className="text-xl font-semibold">Modifier la dépense</h3>
          </div>
          <button 
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                  ...
                  {/* Fournisseur */}
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Fournisseur <span className="text-xs text-gray-500">(optionnel)</span></label>
                    <input
                      type="text"
                      name="fournisseur"

...
interface AjoutVehiculeModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: () => void;
}

const AjoutVehiculeModal: React.FC<AjoutVehiculeModalProps> = ({
  isOpen,
  onClose,
  onSave
}) => {
  ...
  // Soumettre UNIQUEMENT depuis le bouton final
  const handleFinalSubmit = async () => {
    ...
    try {
      ...
      if (!id_ecole) {
        alert("ID de l'auto-école non disponible");
        setLoading(false);
        return;
      }
      ...
      const dataToSend = {
        ...formData,
        id_ecole,
        id_bureau: id_bureau === 0 ? null : id_bureau,
      };
      
      // Appeler l'API
      const response = await fetch('/directeur/vehicules/components/AjoutVehicule/api', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        ...
        alert(errorData.error || 'Erreur lors de l\'ajout du véhicule');
        setLoading(false);
        return;
      }
        ...
        <div className="fixed top-4 right-4 z-[60] animate-fade-in">
          <div className="bg-green-500 text-white px-6 py-4 rounded-lg shadow-lg flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-sm">Le véhicule a été ajouté avec succès</p>
            </div>
          </div>
        </div>
      )}

...
interface Utilisateur {
  ...
  role: 'directeur' | 'moniteur' | 'secretaire' | 'comptable';
  created_at: string;
  updated_at: string;
}
...
async function mapToUserResponse(utilisateur: Utilisateur): Promise<UserResponse> {
  ...
  return {
    ...
    id_bureau: utilisateur.id_bureau,
...
export async function GET(request: Request) {
  try {
    ...
    // Filtrer par école
    query = query.eq('id_ecole', parseInt(idEcole));
...
export async function POST(request: Request) {
  try {
    ...
    if (!userData.nom || !userData.prenom || !userData.email || !userData.role || !userData.id_ecole) {
      return NextResponse.json(
    ...
    if (userData.role === 'moniteur') {
      // Convertir la date de délivrance en format ISO si elle existe
      let dateDelivranceFormatted = null;
      if (userData.date_delivrance_num && userData.date_delivrance_num.trim() !== '') {
        try {
          dateDelivranceFormatted = new Date(userData.date_delivrance_num).toISOString().split('T')[0];
        } catch (error) {
      ...
      const { error: enseignantError } = await supabase
        ...
        .insert({
          ...
          date_delivrance_num: dateDelivranceFormatted,
          id_ecole: userData.id_ecole,
          id_bureau: userData.id_bureau || null,
          id_utilisateur: userId
        });
...
export async function PUT(request: Request) {
  try {
    ...
    // Validation des données
    if (!userData.id || !userData.nom || !userData.prenom || !userData.email || !userData.role) {
      return NextResponse.json(
        { error: 'Tous les champs obligatoires doivent être remplis' },
        { status: 400 }
      );
    ...
    if (utilisateurError) {
      console.error('Erreur lors de la mise à jour de l\'utilisateur:', utilisateurError);
      return NextResponse.json(
    ...
    if (userData.role === 'moniteur') {
      ...
      if (existingEnseignant) {
        ...
        if (enseignantError) {
          console.error('Erreur lors de la mise à jour de l\'enseignant:', enseignantError);
          // Ne pas retourner d'erreur ici, car l'utilisateur a été mis à jour avec succès
        }
      } else {
        // Convertir la date de délivrance en format ISO si elle existe
        let dateDelivranceFormatted = null;
...
export async function DELETE(request: Request) {
  try {
    ...
    if (utilisateur?.role === 'moniteur') {
      ...
      if (enseignantError) {
        console.error('Erreur lors de la suppression de l\'enseignant:', enseignantError);
        // Ne pas retourner d'erreur ici, continuer avec la suppression de l'utilisateur
      }
    }
    ...
    if (utilisateurError) {
      console.error('Erreur lors de la suppression de l\'utilisateur:', utilisateurError);
      return NextResponse.json(
    ...
    if (authError) {
      ...
      // Ne pas bloquer la réponse si la suppression dans auth échoue
    }
